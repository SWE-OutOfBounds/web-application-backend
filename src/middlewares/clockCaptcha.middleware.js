...
const clockCAPTCHAService = require("../services/clockCAPTCHA.service");
  ...
  validate: async (req, res, next) => {
    ...
    if (token && user_input) {
      ...
      try {
        ...
        const tokenFound = await clockCAPTCHAService.searchForToken(
          decoded_token
        );
        ...
        if (!tokenFound) {
          const blackListToken = await clockCAPTCHAService.insertToken(
            decoded_token
          );
